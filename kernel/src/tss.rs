...
pub fn init(mm: &impl PageAllocator) -> &'static TaskStateSegment {
    let df_stack = mm
        .map(DOUBLE_FAULT_STACK_SIZE, PageTableFlags::PRESENT | PageTableFlags::WRITABLE)
        .expect("failed to allocate double fault stack");

    let ring0_stack = mm
        .map(RING0_STACK_SIZE, PageTableFlags::PRESENT | PageTableFlags::WRITABLE)
