...
macro_rules! set_handler {
    ...
    ($idt:ident, $name:ident, stack = $index:expr) => {
        unsafe { set_handler!($idt, $name).set_stack_index($index) }
    };
}
...
pub fn init() {
    ...
    set_handler!(idt, debug, stack = tss::DEBUG_IST_INDEX);
    set_handler!(
        idt,
        non_maskable_interrupt,
        stack = tss::NM_PANICKING_IST_INDEX
    );
    ...
    set_handler!(idt, double_fault, stack = tss::DOUBLE_FAULT_IST_INDEX);
    ...
    set_handler!(idt, page_fault, stack = tss::NM_PANICKING_IST_INDEX);
    set_handler!(idt, x87_floating_point);
    set_handler!(idt, alignment_check);
    set_handler!(idt, machine_check, stack = tss::NM_PANICKING_IST_INDEX);
    ...
    unsafe { idt[32].set_handler_addr(VirtAddr::new(handler::timer_interrupt as u64)) };

    unsafe { IDT.finish_init(idt) };
