...
pub trait MemoryManager {
    ...
    fn mremap(&mut self, handle: MappingHandle, new_size: usize, new_flags: PageTableFlags) -> Option<MappingHandle>;
}
    ...
    fn mremap(&mut self, handle: MappingHandle, new_size: usize, new_flags: PageTableFlags) -> Option<MappingHandle> {
        ...
        if handle.flags != new_flags {
            for i in 0..keep_count {
                let virt = VirtAddr::new(handle.start.as_u64() + (i * L0_PAGE_SIZE) as u64);
                if let Some(frame) = self.addr_space.unmap(virt) {
                    self.addr_space.map(virt, frame, new_flags, &self.frame_alloc).ok()?;
                }
            }
        }
        ...
        if new_page_count < old_page_count {
            ...
            let mapping_end = VirtAddr::new(handle.start.as_u64() + (old_page_count * L0_PAGE_SIZE) as u64);
            ...
            self.virt_next = VirtAddr::new(handle.start.as_u64() + (new_page_count * L0_PAGE_SIZE) as u64);
        }

        Some(MappingHandle { start: handle.start, size: new_size, flags: new_flags })
    }
}
