...
pub fn init(boot_info: &'static BootInfo) -> BumpAllocator<'static> {
    let region = &boot_info.memory_region;
    let frame_alloc = FrameAllocator::new(region, boot_info.first_usable_phys);
    let mut mm = unsafe {
        BumpAllocator::with_current_pml4(region, frame_alloc, HEAP_VIRT_BASE)
    };

    let heap = mm
        .mmap(HEAP_SIZE, PageTableFlags::PRESENT | PageTableFlags::WRITABLE)
